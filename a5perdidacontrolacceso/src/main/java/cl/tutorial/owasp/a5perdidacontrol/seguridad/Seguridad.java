...
import cl.tutorial.owasp.a5perdidacontrol.util.EncripatadorUtil;
...
public class Seguridad extends WebSecurityConfigurerAdapter {
	...
	private static final String[] SITIOS_PERMITIDOS = { URL_LOGIN, "/logout", "/webjars/**", "/css/**", "/img/**",
			"/js/**" };
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.csrf().disable();
		// permitidos
		http.authorizeRequests().antMatchers(SITIOS_PERMITIDOS).permitAll();
		...
		http.formLogin().loginProcessingUrl(URL_LOGIN).loginPage(URL_LOGIN) //
				...
				.logout().logoutUrl("/logout").logoutSuccessUrl("/login?sesion");
	}
}
